...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajouter_support'])) {
    $_nom    = trim($_POST['nom']    ?? '');
    ...
    if (empty($_erreurs)) {
        ...
        $_ret  = $_supportDAO->ajouterSupport($_nom, $_prenom, $_email, $_hash);
        if ($_ret > 0) {
            $_succes = 'Support créé.';
        } elseif ($_ret === -1) {
...
$_total   = $_supportDAO->compterSupports();
$_nbPages = (int) max(1, ceil($_total / $_perPage));
if ($_page > $_nbPages) {
    $_page   = $_nbPages;
    $_offset = ($_page - 1) * $_perPage;
}
$_supports = $_supportDAO->getSupportsPagine($_offset, $_perPage) ?? [];
?>

<div class="container-fluid p-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h3 class="fw-bold mb-0">
            <i class="bi bi-headset me-2"></i>Supports
        </h3>
        <button class="btn btn-primary" data-bs-toggle="modal"
                data-bs-target="#modalAjoutSupport">
            <i class="bi bi-plus-circle me-2"></i>Nouveau support
        </button>
    </div>
    ...
    <p class="text-muted small mb-3">
        <?= $_total ?> support(s) — page <?= $_page ?> / <?= $_nbPages ?>
    </p>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($_supports as $_s): ?>
                            <tr>
                                <td class="text-muted small">
                                    <?= (int) $_s['id_support'] ?>
                                </td>
                                <td><?= htmlspecialchars($_s['nom_support']) ?></td>
                                <td><?= htmlspecialchars($_s['prenom_support']) ?></td>
                                <td><?= htmlspecialchars($_s['email_support']) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
        ...
        <nav class="mt-3">
            <ul class="pagination justify-content-center mb-0">
                <li class="page-item <?= $_page <= 1 ? 'disabled' : '' ?>">
                    <a class="page-link"
                       href="/root/index_.php?page=gestion_supports&p=<?= $_page - 1 ?>">
                        Précédent
                    </a>
                </li>
    ...
    <div class="modal-dialog">
        <form method="post" class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title">Nouveau support</h5>
            ...
            <div class="modal-footer">
                ...
                <button type="submit" name="ajouter_support"
